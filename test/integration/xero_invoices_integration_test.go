...
func TestInvoicesOnlineURLIntegrationRefreshesThenCallsXeroAPI(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestInvoicesApproveIntegrationRefreshesThenCallsXeroAPI(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api.xro/2.0/Invoices" {
			t.Fatalf("unexpected path: %s", r.URL.Path)
		}
		...
		if string(body) != `{"Invoices":[{"InvoiceID":"220ddca8-3144-4085-9a88-2d72c5133734","Status":"AUTHORISED"}]}` {
			t.Fatalf("unexpected request body: %s", string(body))
		}
		_, _ = io.WriteString(w, `{"Invoices":[{"InvoiceID":"220ddca8-3144-4085-9a88-2d72c5133734","Type":"ACCREC","InvoiceNumber":"INV-1000","Status":"AUTHORISED","UpdatedDateUTC":"2026-03-11T12:30:00Z"}]}`)
	}))
	defer server.Close()
	...
	if !bytes.Contains(stdout.Bytes(), []byte(`"invoiceNumber": "INV-1000"`)) {
		t.Fatalf("expected approval output, got %s", stdout.String())
	}
	if !bytes.Contains(stdout.Bytes(), []byte(`"summary": "invoice approved"`)) {
